public static class GameResourcePaths
{
    public const string EnemyGrunt = "Prefabs/Enemies/Enemy_Grunt";
    public const string EnemyFast = "Prefabs/Enemies/Enemy_Fast";
    public const string EnemyTank = "Prefabs/Enemies/Enemy_Tank";

    public const string XPSmallOrb = "Prefabs/Drops/XPOrb_Small";
    public const string XPMediumOrb = "Prefabs/Drops/XPOrb_Medium";
    public const string XPLargeOrb = "Prefabs/Drops/XPOrb_Large";

    public const string FloatingDamageText = "Prefabs/UI/FloatingDamageText";
    public const string PlayerBullet = "Prefabs/Bullets/PlayerBullet";
}